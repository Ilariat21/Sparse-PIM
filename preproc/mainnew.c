...
int main(int argc, char *argv[]) {
    ...
    int *vec1 = malloc(Tf * Tf * sizeof(MTYPE));
    ...
    for (int i=0; i<Tf; i++){
        ...
        for (int j=i; j<Tf; j++){
            ...
            for (int k=1; k<Kdim+1; k++){
                // printf("info: %d %d %d\n", i, j, k);
                printf("tiles[i].col_ptr[k] %d %d\n", tiles[i].col_ptr[k], tiles[j].col_ptr[k]);
                tempval += (tiles[i].col_ptr[k] - tiles[i].col_ptr[k-1]) * (tiles[j].col_ptr[k] - tiles[j].col_ptr[k-1]); 
                }
            printf("temp value %d %d \n", i*Tf+j, tempval);
            vec1[i*Tf+j] = tempval;
            printf("temp value %d %d \n", i*Tf+j, vec1[i*ntiles+j]);
            }
        }
    ...
    for (int t = 0; t < ntiles; t++) {
        free(tiles[t].col_ptr);
        free(tiles[t].row_idx);
        free(tiles[t].data);
    }
    free(tiles);
