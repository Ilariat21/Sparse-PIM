...
SparsePIM Accelerator functional simulator / trace generator
                        Initially written by (uegook 11/04/2026)
...
def place_ptrs(ptr_list, bank, start_row):
    addr_ptr = 0
    col, row, ptr = 0, start_row, 0
    for i in range(len(ptr_list)):
        mem_set(ptr_list[i], bank, row, col, ptr)
        addr_ptr += 1
        ptr = addr_ptr % DRAM_COLSZ
        col = (addr_ptr // DRAM_COLSZ) % DRAM_NCOLS
        row = (addr_ptr // DRAM_COLSZ) // DRAM_NCOLS + start_row
...
def place_nzs(nz_list1, nz_list2, bank, start_row):
    col, row, ptr = 0, start_row, 0
    for i in range(len(nz_list1)):
        mem_set(nz_list1[i], bank, row, col, ptr)
        mem_set(nz_list2[i], bank, row, col+1, ptr)
        ptr += 1
        if ptr >= DRAM_COLSZ:
            col, ptr = col+2, 0
            if col >= DRAM_NCOLS:
