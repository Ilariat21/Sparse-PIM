...
void Config::CalculateSize() {
    ...
    std::cout << "devices_per_rank:" << devices_per_rank << "\npage_size:" << page_size << " \nmegs_per_bank:" << megs_per_bank << " \nmegs_per_rank:" << megs_per_rank << "\n" << std::endl;
...
void Config::InitDRAMParams() {
    ...
    std::cout << "banks: " << banks << "\tbankgroups: " << bankgroups << "\tbanks_per_group: " << banks_per_group << "\n" << std::endl;
...
void Config::SetAddressMapping() {
    ...
    std::cout << "field_widths\n" << std::endl;
    std::cout << "ch:" << field_widths["ch"] << " ra:" << field_widths["ra"] << " bg:" << bg_mask << " ba:" << field_widths["ba"] << " ro:" << field_widths["ro"] << " co:" << field_widths["co"] << std::endl;
    ...
    for (size_t i = 0; i < address_mapping.size(); i += 2) {
        ...
        std::string combinedFields;
        for (const std::string& field : fields) {
            combinedFields += field + " ";
        }        
        printf("fields: %s\n", combinedFields.c_str());
    }
    ...
    // print("ch_pos:%s ra_pos:%s bg_pos:%s ba_pos:%s ro_pos:%s co_pos:%s\n", ch_pos.c_str(), ra_pos.c_str(), bg_pos.c_str(), ba_pos.c_str(), ro_pos.c_str(), co_pos.c_str());
    std::cout << "ch_pos:" << ch_pos << " ra_pos:" << ra_pos << " bg_pos:" << bg_pos << " ba_pos:" << ba_pos << " ro_pos:" << ro_pos << " co_pos:" << co_pos << std::endl;
    //ch_pos:5 ra_pos:7 bg_pos:6 ba_pos:5 ro_pos:7 co_pos:0
    ...
    std::cout << "ch_mask:" << ch_mask << " ra_mask:" << ra_mask << " bg_mask:" << bg_mask << " ba_mask:" << ba_mask << " ro_mask:" << ro_mask << " co_mask:" << co_mask << std::endl;
