...
itrs = int((tiling_factor_m*tiling_factor_n)**0.5)
...
for i in range(itrs):
    print("iter", i)
    temparr = MKarr[i*math.ceil(Mdim/tiling_factor_m):min(Mdim, (i+1)*math.ceil(Mdim/tiling_factor_m)),:]
    vals, idxs, ptrs = compress_array_to_csc(temparr)
    print("Head:", i, vals[:5], idxs[:5], ptrs[:5])                                                       
...
for i in range(len(pointers_stn)):
    print("i", i)
    nz_coli = []
    for ki in range(len(pointers_stn[i])-1):
        nz_coli.append(pointers_stn[i][ki+1] - pointers_stn[i][ki])
    for j in range(len(pointers_stn)):
        nz_colj = []
        for kj in range(len(pointers_stn[j])-1):
            nz_colj.append(pointers_stn[j][kj+1] - pointers_stn[j][kj])
        rez_nz = []
        for k in range(len(nz_coli)):
            rez_nz.append(nz_coli[k]*nz_colj[k])
        rezz.append(sum(rez_nz))
print("Here is the effectual banks", rezz)
                                        ...
                                        for vr in range(vrf.shape[0]):
                                            for vc in range(vrf.shape[1]):
                                                ...
                                                if vr+vvr < len(stn_row_buffer) and vc+vvc < len(str_col_buffer):
                                                    ...
                                                    vrf[vr, vc] = out_buff[stn_row_buffer[vr+vvr], str_col_buffer[vc+vvc]]

                                        util += 1
                                        for r in range(min(vrf.shape[0], len(stn_row_buffer)-vvr)):
                                            for t in range(min(vrf.shape[1], len(str_col_buffer)-vvc)):
                                                temp_val = stn_val_buffer[r+vvr]*str_val_buffer[t+vvc]
                                                mac_cnt += 1
                                                vrf[r, t] = vrf[r, t] + temp_val
                                                eff += 1
                                        for vr in range(vrf.shape[0]):
                                            for vc in range(vrf.shape[1]):
                                                # cycle_inc(1)
                                                if vr+vvr < len(stn_row_buffer) and vc+vvc < len(str_col_buffer):
                                                    out_buff[stn_row_buffer[vr+vvr], str_col_buffer[vc+vvc]] = vrf[vr, vc]
...
# num_MK_subtiles = math.ceil(Mdim/tau_m)
# num_KN_subtiles = math.ceil(Ndim/tau_n)

# MKsubtiles_per_bank = math.ceil(num_MK_subtiles/tiling_factor_m)
# KNsubtiles_per_bank = math.ceil(num_KN_subtiles/tiling_factor_n)
...
# for t in range(num_MK_subtiles):
#     nnzc = 0
#     temparr = MKarr[t*tau_m : min(MKarr.shape[0], tau_m*(1+t)), :]
#     for i in range(temparr.shape[0]):
#         for j in range(temparr.shape[1]):
#             if temparr[i, j] != 0.0:
#                 nnzc += 1
#     MKsubtile_nnz.append(nnzc)

# MKid = 0
# MKpergroup = 0
# for i in range(0, len(MKsubtile_nnz), MKsubtiles_per_bank):
#     total = 0
#     for j in range(MKsubtiles_per_bank):
#         if i+j < len(MKsubtile_nnz):
#             total += MKsubtile_nnz[i+j]
#     print("total", i//MKsubtiles_per_bank, total)
#     if total >= MKpergroup:
#         MKpergroup = total
#         MKid = i//MKsubtiles_per_bank

# print("MK", MKpergroup, MKid)

# print(MKid*MKsubtiles_per_bank*tau_m, min(MKarr.shape[0], (MKid+1)*MKsubtiles_per_bank*tau_m))
# MKarr = MKarr[MKid*MKsubtiles_per_bank*tau_m:min(MKarr.shape[0], (MKid+1)*MKsubtiles_per_bank*tau_m), :]


# for t in range(num_KN_subtiles):
#     nnzc = 0
#     temparr = KNarr[:, t*tau_n : min(KNarr.shape[1], tau_n*(1+t))]
#     for i in range(temparr.shape[0]):
#         for j in range(temparr.shape[1]):
#             if temparr[i, j] != 0.0:
#                 nnzc += 1
#     KNsubtile_nnz.append(nnzc)

# KNid = 0
# KNpergroup = 0
# for i in range(0, len(KNsubtile_nnz), KNsubtiles_per_bank):
#     total = 0
#     for j in range(KNsubtiles_per_bank):
#         if i+j < len(KNsubtile_nnz):
#             total += KNsubtile_nnz[i+j]
#     print("total", i//KNsubtiles_per_bank, total)
#     if total >= KNpergroup:
#         KNpergroup = total
#         KNid = i//KNsubtiles_per_bank

# print("KN", KNpergroup, KNid)

# print(KNid*KNsubtiles_per_bank*tau_n, min(KNarr.shape[1], (KNid+1)*KNsubtiles_per_bank*tau_n))
# KNarr = KNarr[:, KNid*KNsubtiles_per_bank*tau_n:min(KNarr.shape[1], (KNid+1)*KNsubtiles_per_bank*tau_n)]
...
# cpu_nnz = 0
# for i in range(cpu_res.shape[0]):
#     for j in range(cpu_res.shape[1]):
#         if cpu_res[i,j]!=0.0:
#             cpu_nnz += 1
# print("cpu nnz", cpu_nnz)
