...
SparsePIM Accelerator functional simulator / trace generator
                      Initially written by (ms1021 01/Sep./2025)
...
# parser.add_argument('-mat', '--mat', type=str, default='/home/97ms/1.research/sparse/1.gp-pim/mat_example.mtx', help='Input matrix file')
...
def place_ptrs(ptr1, bank, start_row):  # return next start row after place ptrs on DRAM
    col, row, ptr = 0, start_row, 0
    for i in range(len(ptr1)):
        mem_set(ptr1[i], bank, row, col, ptr)
        ptr += 1
...
# FOR ALL subtile
ptr_rows_stn = [0] # list of DRAM rows where each sub-tile pointers start
# save all start row while save MAT A pointers into DRAM bank
print(f"pointers_stn: {ptr_rows_stn[0]}")
for i in range(len(pointers_stn)): # Mat A pointers for each sub-tiles
    ptr_rows_stn.append(place_ptrs(pointers_stn[i], bank, ptr_rows_stn[-1])) # save DRAM start row idx

ptr_rows_str = [ptr_rows_stn[-1]] # use last value of ptr_rows_stn as start row of MAT B
...
# ptr_rows_stn: list of start rows
...
for i in tqdm(range(len(ptr_rows_stn)-1)):
    ...
    for r1 in range(ptr_rows_stn[i], ptr_rows_stn[i+1]): # load every row between sub-tile 1 start row and 2 start row
            # print(f"Working on row {r1} / {ptr_rows_stn[i]}, {ptr_rows_stn[i+1]}")
        stn_val_addr_list=[]
        stn_idx_addr_list=[]
        tiled_stn_ptr = []
        if flag ==1:
        ...
        for c1 in range(DRAM_NCOLS):
            ...
            for k in range(len(stn_ptr_chunk)):
                if flag == 1:
                    break
                if last_stn_ptr==-1:
                    last_stn_ptr = stn_ptr_chunk[k]
                    continue
                stn_nnz = stn_ptr_chunk[k] - last_stn_ptr

                if stn_nnz < 0:
                    flag = 1
                    print(f"{stn_ptr_chunk}")
                    # time.sleep(1)
                    break
                else:
                    # stn_ptr.append(stn_ptr_chunk[k])
                    tiled_stn_ptr.append(stn_ptr_chunk[k])
                
                if stn_nnz>0:
                    print(f"")
                    stn_val_addr, stn_idx_addr = addr_gen(last_stn_ptr, stn_ptr_chunk[k], val_rows_stn[i])
                    print(f"start: {last_stn_ptr}, end: {stn_ptr_chunk[k]}, val_row: {val_rows_stn[i]}")
                    for addr in range(len(stn_val_addr)):
                        print(f"val: {read_mem_to_buf(bank, stn_val_addr[addr][0], stn_val_addr[addr][1])}")
                    for addr in range(len(stn_idx_addr)):
                        print(f"idx: {read_mem_to_buf(bank, stn_idx_addr[addr][0], stn_idx_addr[addr][1])}")
                    # print(f"stn_val_addr: {stn_val
                    stn_val_addr_list.append(stn_val_addr)
                    stn_idx_addr_list.append(stn_idx_addr)
                print("\n\n#########################################")
                print(f"stn_val_addr:{stn_val_addr}\nstn_idx_addr:{stn_idx_addr}")
                print("#########################################\n\n")
                # time.sleep(1)
                last_stn_ptr = stn_ptr_chunk[k]
            print("stn_val_addr_list:", stn_val_addr_list)
            print("stn_idx_addr_list:", stn_idx_addr_list)

                # if last_stn_ptr==-1:
                #     last_str_ptr, last_stn_ptr = str_ptr_chunk[k], stn_ptr_chunk[k]

                # if k != len(stn_ptr_chunk) - 1:
                #     stn_row_nnz = stn_ptr_chunk[k+1] - stn_ptr_chunk[k]
                    # print(f"STN: NNZ on row {r1}, col {c1}: {stn_row_nnz}")
        ...
        # print("stn_ptr_chunk: ", stn_ptr_chunk)
        # print("tiled_stn_ptr:", tiled_stn_ptr)

        # for k in range(len(tiled_stn_ptr)-1):
        #     if k == 0:
        #         stn_row_nnz_tmp = tiled_stn_ptr[0]
        #         # stn_row_nnz는 row당 nnz 갯수...
        #         stn_row_nnz.append(stn_row_nnz_tmp)
        #         print(f"STN: NNZ on row {r1}, col {c1}: {stn_row_nnz_tmp}")
        #     stn_row_nnz_tmp = tiled_stn_ptr[k+1] - tiled_stn_ptr[k]
        #     stn_row_nnz.append(stn_row_nnz_tmp)
        #     print(f"STN: NNZ on row {r1}, col {c1}: {stn_row_nnz_tmp}")
        # print(f"stn_row_nnz: {stn_row_nnz}")
        # # stn_row_nnz_list.append(stn_row_nnz)
        # # tiled_stn_ptr는 타일당 
        # # stn_ptr.append(tiled_stn_ptr) 
        # # print("waiting for 3 sec...")
        # # print(stn_ptr)
        # # print(stn_row_nnz_list)
        # # time.sleep(1)
        # # load
        ...
        # # 
        # # 
        # # 1. stn_val & stn_idx load
        
        # print(stn_row_nnz)
        # time.sleep(1)
        # for kk in range(len(stn_row_nnz)):
        #     print(f"len(stn_row_nnz): {len(stn_row_nnz)}, {stn_row_nnz[kk]}")
        #     time.sleep(1)
        #     row_accum = {}
        #     # for ll in range(stn_row_nnz[kk]):
        #     print(stn_val_addr_list)
        #     for k in range(len(stn_val_addr_list)):
                
        #         stn_val_addr = stn_val_addr_list[k]
        #         print(f"stn_val_addr: {len(stn_val_addr)}, {stn_val_addr}")
        #         time.sleep(1)
        #         for l in range(len(stn_val_addr_list[k])):
        ...
        #                     print("READ")
        #                     # print(read_mem_to_buf(bank, stn_val_addr[p+n][0], stn_val_addr[p+n][1]))
        #                     # stn_val_buffer.append(read_mem_to_buf(bank, stn_val_addr[p+n][0], stn_val_addr[p+n][1]))
        #                     print(read_mem_to_buf(bank, stn_val_addr[n][0], stn_val_addr[n][1]))
        #                     stn_val_buffer.append(read_mem_to_buf(bank, stn_val_addr[n][0], stn_val_addr[n][1]))
        #                 print(f"stn_val_buffer: {stn_val_buffer}")
        #                 for n in range(last_stn):
        ...
        #                 print(f"\n\nstn_val_buffer:{stn_val_buffer}")
        #                 print(f"stn_row_buffer:{stn_row_buffer}\n\n")
        #                 time.sleep(2)

        #     # for row, nnz_num in enumerate(stn_row_nnz):
        #     #     str_val_addr_list
            

        # print(f"len: {len(stn_ptr)}")
        # # time.sleep(2)
        

        # #MAT A의 row 하나에 대한 처리가 끝났음.
        # #이제 mat A의 idx를 보고 MAT B의 각 row를 불러와야 함
        # #tiled_stn_ptr
        ...
        #     for r2 in range(ptr_rows_str[j], ptr_rows_str[j+1]): #  row by row for MAT B
        #         # tiled_str_ptr=[]
        #         for c2 in range(DRAM_NCOLS):
        #             if flag ==1:
        #                 break
        #             str_ptr_chunk = read_mem_to_buf(bank, r2, c2)
        #             # stn_nnz = stn_ptr_chunk[k] - last_stn_ptr
        ...
        #             #todo: find 
        #             print(f"STR: {str_ptr_chunk}")

        #             # tiled_stn_ptr = list()

        #             for k in range(len(str_ptr_chunk)):
        #                 if flag == 1:
        #                     break
        #                 if last_str_ptr==-1:
        #                     last_str_ptr = str_ptr_chunk[k]
        #                     continue
        #                 str_nnz = str_ptr_chunk[k] - last_str_ptr

        #                 if str_nnz < 0:
        #                     flag = 1
        #                     print(f"{str_ptr_chunk}")
        #                     # time.sleep(1)
        #                     break
        #                 else:
        #                     # stn_ptr.append(stn_ptr_chunk[k])
        #                     tiled_str_ptr.append(str_ptr_chunk[k])

        #                 if str_nnz>0:
        #                     str_val_addr, str_idx_addr = addr_gen(last_str_ptr, str_ptr_chunk[k], val_rows_str[j])

        #                 print(f"str_val_addr:{str_val_addr}\nstr_idx_addr:{str_idx_addr}")
        #                 # time.sleep(1)
        #                 last_str_ptr = str_ptr_chunk[k]
        #         # for k in range(len(tiled_stn_ptr)):
        #     for k in range(len(stn_idx_addr)):
        #         print(f"stn_idx_addr: {stn_idx_addr}")
        #         str_idx_chunk = read_mem_to_buf(bank, stn_idx_addr[k][0], stn_idx_addr[k][1])
        #         print(f"str_idx_chunk: {str_idx_chunk}")
                ...
                #     for k in range(len(str_ptr_chunk)-1):
                        
                #         stn_row_nnz = str_ptr_chunk[k+1] - str_ptr_chunk[k]
                #         print(f"STR: NNZ on row {r1}, col {c1}: {stn_row_nnz}")

                # str_idx_chunk = read_mem_to_buf(bank,r2 , c2)
            ...
            #         break
                ...
                # find difference of DRAM row..
                # r2 = r1 + (j-i)*ptr_len + ptr_rows_stn[-1] # <-- OP need read same row of B
                                                           # for outer product dataflow, I need to read same index of A and B pointers,
                ...
                #                 for m in range(last_str):
                #                     str_row_buffer.append(read_mem_to_buf(bank, str_idx_addr[l+m][0], str_idx_addr[l+m][1]))
                ...
                #                 str_row_buffer = np.concatenate(str_row_buffer).tolist()[start_str_idx:end_str_idx]
                #                 # print("bb",str_row_buffer)
                ...
                #                     for vvc in range(0, len(str_row_buffer), vrf.shape[1]):
                #                         for vr in range(vrf.shape[0]):
                #                             for vc in range(vrf.shape[1]):
                #                                 # cycle_inc(1)
                #                                 if vr+vvr < len(stn_row_buffer) and vc+vvc < len(str_row_buffer):
                #                                     # print("aa", stn_row_buffer[vr+vvr], str_row_buffer[vc+vvc])
                #                                     vrf[vr, vc] = out_buff[stn_row_buffer[vr+vvr], str_row_buffer[vc+vvc]]  # get data from Output buffer to VRF

                #                         util += 1
                #                         for r in range(min(vrf.shape[0], len(stn_row_buffer)-vvr)):
                #                             for t in range(min(vrf.shape[1], len(str_row_buffer)-vvc)):
                #                                 temp_val = stn_val_buffer[r+vvr]*str_val_buffer[t+vvc] # mult
                #                                 mac_cnt += 1
                #                                 vrf[r, t] = vrf[r, t] + temp_val # acc
                #                                 eff += 1
                ...
                #                                 # cycle_inc(1)
                #                                 if vr+vvr < len(stn_row_buffer) and vc+vvc < len(str_row_buffer):
                #                                     out_buff[stn_row_buffer[vr+vvr], str_row_buffer[vc+vvc]] = vrf[vr, vc] # get data from VRF to Output buffer
